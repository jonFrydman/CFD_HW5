...
void airfoilFlux() { //take the pressure from the two cells above the foil and linearly

	InterpolatedPressure = cellset[i][0].P(); //Mirrored Pressure

	F(1) = 0;
	F(2) = cellset[][].U()*InterpolatedPressure;
	F(3) = cellset[][].V()*InterpolatedPressure;
	F(4) = 0;
...
vector<cellState> stencilEW(vector< vector<cellState> > cellset, int i, int j) {
	...
	else if (i + 3>cellset.size() - 1) { //Stencil focused near the I=IMAX border
		int cnt = i - 3;
		int n = 0;
		while (n<7) {
